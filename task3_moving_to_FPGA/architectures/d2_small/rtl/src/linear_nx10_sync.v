...
module linear_nx10_sync #(
    parameter integer IN_FEATURES = 64,
    ...
    reg [2:0] state;
    reg [3:0] class_idx;
    reg [INDEX_WIDTH-1:0] feature_idx;
    reg signed [31:0] acc;
    /* verilator lint_off UNUSEDSIGNAL */
    reg [31:0] weight_addr_full;
    /* verilator lint_on UNUSEDSIGNAL */
    wire signed [31:0] product32 = $signed(feature_data) * $signed(weight_data);
    wire last_feature = &feature_idx;
    wire last_class = (class_idx == 4'd9);

    always @(*) begin
        feature_addr = {{(FEATURE_ADDR_WIDTH-INDEX_WIDTH){1'b0}}, feature_idx};
        weight_addr_full = class_idx * IN_FEATURES + {{(32-INDEX_WIDTH){1'b0}}, feature_idx};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state<=S_IDLE; class_idx<=0; feature_idx<=0; acc<=0; logit_index<=0;
            logit_data<=0; logit_valid<=0; busy<=0; done<=0;
        end else begin
            logit_valid<=0; done<=0;
            case (state)
                S_IDLE: begin busy<=0; if (start) begin class_idx<=0; feature_idx<=0; acc<=0; busy<=1; state<=S_ADDR; end end
                S_ADDR: state<=S_ACC;
                S_ACC: begin acc<=acc+product32; if (last_feature) state<=S_EMIT;
                       else begin feature_idx<=feature_idx+1'b1; state<=S_ADDR; end end
                // Bias uses the same input_scale*weight_scale as the accumulator.
                // bias 与累加器采用相同 scale，因此此处无需再次重量化。
                S_EMIT: begin logit_index<=class_idx; logit_data<=acc+bias_data; logit_valid<=1;
                    if (last_class) state<=S_DONE;
                    else begin class_idx<=class_idx+1'b1; feature_idx<=0; acc<=0; state<=S_ADDR; end end

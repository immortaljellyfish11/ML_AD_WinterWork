...
module multi_channel_conv3x3_sync #(
    ...
    assign product16 = valid_pixel ? ($signed(feature_data) * $signed(weight_data)) : 16'sd0;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_READ: begin
                    // BRAM reads the next feature/weight while the previous
                    // registered product is accumulated.
                    if (product_valid) begin
                        raw_sum <= raw_sum + {{16{product_reg[15]}}, product_reg};
                ...
                S_MUL: begin
                    product_reg <= product16;
                    product_valid <= 1'b1;
                    product_last <= last_item;
                    if (!last_item) begin
                        if (kc != 2'd2) begin
                            kc <= kc + 2'd1;
                        end else begin

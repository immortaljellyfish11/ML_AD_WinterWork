...
// D2 16-32-64 INT8 top with synchronous ROM and ping-pong feature buffers.
// D2 16-32-64 INT8 顶层，使用同步 ROM 与乒乓特征缓冲完成整网推理。
module d2_small_top #(
    parameter integer ADDR_WIDTH = 17,
    parameter integer FEATURE_A_DEPTH = 8192,
    parameter integer FEATURE_B_DEPTH = 16384,
    parameter integer FRAC_BITS = 7,
    parameter CONV1_WEIGHT_FILE = "conv1_weight.mem",
    parameter CONV2_WEIGHT_FILE = "conv2_weight.mem",
    parameter CONV3_WEIGHT_FILE = "conv3_weight.mem",
    parameter LINEAR_WEIGHT_FILE = "linear_weight.mem",
    parameter CONV1_BIAS_FILE = "conv1_bias.mem",
    parameter CONV2_BIAS_FILE = "conv2_bias.mem",
    parameter CONV3_BIAS_FILE = "conv3_bias.mem",
    parameter LINEAR_BIAS_FILE = "linear_bias.mem",
    parameter integer CONV1_REQUANT_MULTIPLIER = 35506,
    parameter integer CONV1_REQUANT_SHIFT = 24,
    parameter integer CONV2_REQUANT_MULTIPLIER = 53710,
    parameter integer CONV2_REQUANT_SHIFT = 24,
    parameter integer CONV3_REQUANT_MULTIPLIER = 54661,
    parameter integer CONV3_REQUANT_SHIFT = 24
) (
    input clk,
    input rst_n,
    input start,
    ...
    localparam integer CONV1_WEIGHT_DEPTH = 16 * 3 * 3 * 3;
    localparam integer CONV2_WEIGHT_DEPTH = 32 * 16 * 3 * 3;
    localparam integer CONV3_WEIGHT_DEPTH = 64 * 32 * 3 * 3;
    localparam integer LINEAR_WEIGHT_DEPTH = 10 * 64;
    localparam integer CONV1_BIAS_DEPTH = 16;
    localparam integer CONV2_BIAS_DEPTH = 32;
    localparam integer CONV3_BIAS_DEPTH = 64;
    ...
    conv_layer_engine_sync #(
        ...
        .OUT_HEIGHT(32), .OUT_WIDTH(32), .OUT_CHANNELS(16),
        .FRAC_BITS(FRAC_BITS),
        .REQUANT_MULTIPLIER(CONV1_REQUANT_MULTIPLIER), .REQUANT_SHIFT(CONV1_REQUANT_SHIFT),
        .ADDR_WIDTH(ADDR_WIDTH), .BIAS_ADDR_WIDTH(10)
    ) conv1 (
    ...
    pool_layer_2x2_engine_sync #(.IN_HEIGHT(32), .IN_WIDTH(32), .CHANNELS(16), .ADDR_WIDTH(ADDR_WIDTH)) pool1 (
    ...
    conv_layer_engine_sync #(
        .IN_HEIGHT(16), .IN_WIDTH(16), .IN_CHANNELS(16),
        .OUT_HEIGHT(16), .OUT_WIDTH(16), .OUT_CHANNELS(32),
        .FRAC_BITS(FRAC_BITS),
        .REQUANT_MULTIPLIER(CONV2_REQUANT_MULTIPLIER), .REQUANT_SHIFT(CONV2_REQUANT_SHIFT),
        .ADDR_WIDTH(ADDR_WIDTH), .BIAS_ADDR_WIDTH(10)
    ) conv2 (
    ...
    pool_layer_2x2_engine_sync #(.IN_HEIGHT(16), .IN_WIDTH(16), .CHANNELS(32), .ADDR_WIDTH(ADDR_WIDTH)) pool2 (
    ...
    conv_layer_engine_sync #(
        ...
        .OUT_HEIGHT(8), .OUT_WIDTH(8), .OUT_CHANNELS(64),
        .FRAC_BITS(FRAC_BITS),
        .REQUANT_MULTIPLIER(CONV3_REQUANT_MULTIPLIER), .REQUANT_SHIFT(CONV3_REQUANT_SHIFT),
        .ADDR_WIDTH(ADDR_WIDTH), .BIAS_ADDR_WIDTH(10)
    ) conv3 (
    ...
    global_avg_pool_layer_engine_sync #(.IN_HEIGHT(8), .IN_WIDTH(8), .CHANNELS(64), .ADDR_WIDTH(ADDR_WIDTH)) gap (

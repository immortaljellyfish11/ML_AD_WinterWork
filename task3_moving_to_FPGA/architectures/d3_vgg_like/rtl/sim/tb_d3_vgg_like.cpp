...
// 涓枃锛氳鍙?Python 鐢熸垚鐨勬暣鏁板悜閲忥紝閫愭媿鍐欏叆 CHW 杈撳叆缂撳啿锛屽苟鏍稿鍒嗙被鍜屾渶澶?logit銆?// English: Load Python integer vectors, write CHW input cycle by cycle, and check class/max logit.
namespace {
constexpr size_t kInputSize = 3 * 32 * 32;
constexpr uint64_t kTimeout = 500000000ULL;
...
void tick(Vd3_vgg_like_top& top,uint64_t& cycle){top.clk=0;top.eval();top.clk=1;top.eval();cycle++;}
void reset(Vd3_vgg_like_top& top,uint64_t& c){top.start=0;top.input_we=0;top.rst_n=0;for(int i=0;i<5;i++)tick(top,c);top.rst_n=1;tick(top,c);}
}
int main(int argc,char**argv){
    Verilated::commandArgs(argc,argv); auto tests=load("rtl/sim/data/input_int8.txt","rtl/sim/data/golden_logits.txt");
    if(tests.empty())throw std::runtime_error("no test vectors"); Vd3_vgg_like_top top; bool pass=true;
    for(size_t n=0;n<tests.size();n++){uint64_t c=0;reset(top,c);
        ...
        while(!top.done){if(c-begin>kTimeout)throw std::runtime_error("D3 RTL timeout");tick(top,c);}
    ...
    top.final();std::cout<<(pass?"D3 RTL PASS\n":"D3 RTL FAIL\n");return pass?0:1;
}
